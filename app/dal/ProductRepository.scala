...
class ProductRepository @Inject() (dbConfigProvider: DatabaseConfigProvider,
  ...
  def create( 
              ...
              currentAmount: Int, type_1: String, userId: Long, userName: String
    ...
    (tableQ.map(
                  p => (
                          ...
                          p.measureId, p.measureName, p.currentAmount, p.type_1
                        ) 
                ...
                ) += (name, cost, percent, cost + cost * percent, description, measureId, measureName, currentAmount, type_1)
  }
