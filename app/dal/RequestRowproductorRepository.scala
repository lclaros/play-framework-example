...
class RequestRowProductorRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, repoInsum: ProductRepository)(implicit ec: ExecutionContext) {
  ...
  private class RequestRowProductorTable(tag: Tag) extends Table[RequestRowProductor](tag, "requestRowProductor") {
    ...
    def * = (id, requestRowId, productId, productName, productorId, productorName, quantity, precio, paid, status) <> ((RequestRowProductor.apply _).tupled, RequestRowProductor.unapply)
  }

  private val tableQ = TableQuery[RequestRowProductorTable]

  def create(requestRowId: Long, productId: Long, productName: String, productorId: Long, productorName: String, quantity: Int, precio: Double, status: String): Future[RequestRowProductor] = db.run {
    (tableQ.map(p => (p.requestRowId, p.productId, p.productName, p.productorId, p.productorName, p.quantity, p.precio, p.paid, p.status))
    ...
    ) += (requestRowId, productId, productName, productorId, productorName, quantity, precio, 0, status)
  }
  ...
  def update(id: Long, requestRowId: Long, productId: Long, productName: String, productorId: Long, productorName: String, quantity: Int, precio: Double, status: String): Future[Seq[RequestRowProductor]] = db.run {
