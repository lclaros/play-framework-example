...
$ ->
  $.get "/productRequestsByStorekeeper/" + row_id, (rows) ->
    $.each rows, (index, row) ->
      ...
      links = $("<td>").html '<a href="/productRequest_accept/' + row.id + '" class="btn btn-primary">Aceptar</a>' + '<a href="/productRequest_finish/' + row.id + '" class="btn btn-primary">Finalizar</a>' + '<a href="/productRequest_update/' + row.id + '" class="btn btn-primary">Editar</a>' + '<a href="/productRequest_remove/' + row.id + '" class="btn btn-danger">Eliminar</a>' + '<a href="/productRequest_show/' + row.id + '" class="btn btn-info">Mostrar</a>'
