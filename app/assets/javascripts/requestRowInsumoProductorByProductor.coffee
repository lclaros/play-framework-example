...
$ ->
  $.get "/requestRowProductorsByProductor/" + row_id, (rows) ->
    $.each rows, (index, row) ->
      ...
      $("#requestRowProductors").append $("<tr>").append(requestRowId).append(productId).append(productorId).append(quantity).append(price).append(paid).append(status).append(links)
